...
class WireguardDB(db.Model):
    ...
    def __init__(self, userid):
        ...
        for a in conf:
            try:
                res = requests.get(urllib.parse.urljoin(a['url'], 'genkey'), headers={"Authorization": f"Token {a['key']}"}, timeout=2)
...
def load(app):
    ...
    def download():
        ...
        for a in conf:
            try:
                requests.post(urllib.parse.urljoin(a['url'], 'reload'), headers={"Authorization": f"Token {a['key']}"}, json=alluserpriv, timeout=2).text
            except:
                pass
        ...
        with zipfile.ZipFile(sendfile, 'w') as myzip:
            for a in conf:
                try:
                    res = requests.post(urllib.parse.urljoin(a['url'], 'getconfig'), headers={"Authorization": f"Token {a['key']}"}, json=privkey, timeout=2)
                    if res.status_code == 200:
                        userconfig = res.text
